...
- DSCR : jargon bancaire non expliqué signalé par un audit persona UX 2026-08-05 → tooltip explicatif ajouté aux deux endroits où il apparaît en chiffre brut : en-tête triable `th[data-sort-header="dscr"]` de `renderOwnedPortfolioList` (texte fusionné dans l'attribut `title`, pas de `data-tooltip` pour éviter la double bulle avec le tri — desktop uniquement, `owned.html` a sa propre liste mobile simplifiée sans en-têtes) et libellé `.owned-synthese__card-label` "DSCR" de `renderOwnedCalculTab` (via `data-tooltip` + soulignement pointillé discret comme signifiant tactile, convention déjà utilisée pour le badge "Optimal" — cet onglet est partagé PC/mobile, voir le fix tap-friendly dans `utils.js`)
